...
const StreakSystem = require('../utils/streakSystem');
...
class EventController {
    ...
    static async submitEvent(interaction, eventType, description, screenshots, quantity = 1, attendeesPassed = 0) {
        try {
            ...
            if (!PermissionChecker.canSubmitLogs(interaction.member)) {
                ...
                const errorEmbed = MRBEmbeds.createPermissionErrorEmbed();
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            if (!screenshots || screenshots.length === 0) {
                this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                const errorEmbed = MRBEmbeds.createErrorEmbed('At least one screenshot is required!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            if (screenshots.length > 3) {
                this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                const errorEmbed = MRBEmbeds.createErrorEmbed('Maximum 3 screenshots allowed!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            for (let i = 0; i < screenshots.length; i++) {
                ...
                if (!screenshot) {
                    this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                    const errorEmbed = MRBEmbeds.createErrorEmbed(`Screenshot ${i + 1} is missing or invalid!`);
                    return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
                }
                
                if (!screenshot.contentType || !screenshot.contentType.startsWith('image/')) {
                    this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                    const errorEmbed = MRBEmbeds.createErrorEmbed(`Screenshot ${i + 1} must be a valid image file!`);
                    return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
                }
            ...
            if (quantity < 1 || quantity > 20) {
                this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                const errorEmbed = MRBEmbeds.createErrorEmbed('Quantity must be between 1 and 20!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            if (attendeesPassed < 0 || attendeesPassed > 50) {
                this.activeSubmissions.delete(userId); // FIXED: Clean up on early exit
                const errorEmbed = MRBEmbeds.createErrorEmbed('Attendees passed must be between 0 and 50!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            let transactionCommitted = false;  // Track commit status
            ...
            try {
                ...
                let user = await MRBUser.findOne({ discordId: interaction.user.id }).session(session);
                if (!user) {
                    ...
                    if (user.biweeklyPoints === undefined || user.biweeklyPoints === null) user.biweeklyPoints = 0;
                    ...
                    if (user.biweeklyEvents === undefined || user.biweeklyEvents === null) user.biweeklyEvents = 0;
                ...
                // Step 5: Check if user is a booster
                const isBooster = user.isBooster || false;
                ...
                const actualPointsPerEvent = pointsPerEventWithBonus;
                ...
                user.biweeklyPoints = (user.biweeklyPoints || 0) + totalPoints;
                user.allTimePoints = (user.allTimePoints || 0) + totalPoints;
                user.totalEvents = (user.totalEvents || 0) + quantity;
                user.biweeklyEvents = (user.biweeklyEvents || 0) + quantity;
                ...
                if (!user.previousRank || user.previousRank === 0) {
                    const currentRank = await MRBUser.countDocuments({ 
                        biweeklyPoints: { $gt: oldBiWeeklyPoints }
                    }) + 1;
                ...
                // Step 9: Upload screenshots to Cloudinary for permanent storage
                ...
                // Step 10: Create event log with permanent screenshot URLs
                ...
                // Step 11: Create response embed using the freshly saved user data
                ...
                // Step 12: Handle promotion notifications (outside transaction)
                ...
                console.log(`   - Biweekly points: ${oldBiWeeklyPoints} → ${user.biweeklyPoints}`);
                ...
                console.log(`   - Biweekly quota: ${user.biweeklyPoints}/${user.biweeklyQuota} (${user.quotaCompleted ? 'COMPLETED' : 'IN PROGRESS'})`);

            } catch (transactionError) {
                // FIXED: Only rollback if transaction wasn't committed
                if (!transactionCommitted) {
                    await session.abortTransaction();
                    console.error(`📄 TRANSACTION ABORTED for ${interaction.user.username}:`, transactionError);
                } else {
                    console.error(`❌ Error after transaction commit for ${interaction.user.username}:`, transactionError);
                }
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to submit event. Please try again later.');
    ...
    static createEnhancedSubmissionEmbed(user, eventType, description, totalPoints, basePoints, isBooster, screenshots, quantity, attendeesPassed, attendeesBonus) {
        ...
        if (quantity > 1) {
            if (isTryoutEvent) {
                pointsExplanation = `${totalPoints} points\n(${basePoints} base + ${attendeesBonus} attendees × ${quantity} events)`;
            } else {
                pointsExplanation = `${totalPoints} points\n(${basePoints} per event × ${quantity} events)`;
            }
        } else {
            if (isTryoutEvent) {
        ...
        embed.addFields(
            ...
            { 
                name: '📈 Biweekly Progress', 
                value: `${user.biweeklyPoints}/${user.biweeklyQuota} points`, 
                inline: true 
            },
            { 
                name: '🎯 Quota Status', 
        ...
        if (user.quotaCompleted && user.biweeklyPoints - totalPoints < user.biweeklyQuota) {
            embed.setDescription(embed.data.description + '\n\n🎉 **Congratulations! You\'ve completed your biweekly quota!**');
        }
