...
class StatsController {
    ...
    static async getPersonalStats(interaction) {
        try {
            ...
            let user = await MRBUser.findOne({ discordId: interaction.user.id });
            
            // If user doesn't exist, show "no stats" message
            if (!user) {
                const noStatsEmbed = MRBEmbeds.createNoStatsEmbed();
            ...
            if (user.biweeklyPoints === undefined || user.biweeklyPoints === null) {
            ...
            if (user.biweeklyEvents === undefined || user.biweeklyEvents === null) {
            ...
            const statsEmbed = MRBEmbeds.createPersonalStatsEmbed(
                user, 
                interaction.user, 
                biweeklyRank, 
                allTimeRank, 
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve your statistics. Please try again later.');
    ...
    static async getUserStats(interaction, targetUser) {
        try {
            ...
            if (!PermissionChecker.isHR(interaction.member) && targetUser.id !== interaction.user.id) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('You can only view your own stats unless you have HR role.');
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve user statistics. Please try again later.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    }
}
