// controllers/hrController.js - COMPLETE FILE with fixed biweekly reset
...
class HRController {
    ...
    static async managePoints(interaction, targetUser, action, amount, reason) {
        try {
            ...
            if (!PermissionChecker.canManageSystem(interaction.member)) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('Only HR can use this command!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            // Get or create target user
            let user = await MRBUser.findOne({ discordId: targetUser.id });
            if (!user) {
            ...
            const oldBiWeeklyPoints = user.biweeklyPoints || 0;
            ...
            switch (action) {
                case 'add':
                    user.biweeklyPoints = (user.biweeklyPoints || 0) + amount;
                    user.allTimePoints = (user.allTimePoints || 0) + amount;
                    break;
                case 'remove':
                    user.biweeklyPoints = Math.max(0, (user.biweeklyPoints || 0) - amount);
                    user.allTimePoints = Math.max(0, (user.allTimePoints || 0) - amount);
                    break;
                case 'set':
                    const difference = amount - (user.biweeklyPoints || 0);
                    user.biweeklyPoints = amount;
                    user.allTimePoints = Math.max(0, (user.allTimePoints || 0) + difference);
                    break;
                default:
                    const errorEmbed = MRBEmbeds.createErrorEmbed('⚠️ Invalid action. Use: add, remove, set, or remove_all');
                    return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            if (!RankSystem.isExecutiveOrHigher(user.rankLevel)) {
                ...
                if (action === 'add') {
                    ...
                    const difference = amount - oldBiWeeklyPoints;
            ...
            user.biweeklyQuota = currentQuota;
            ...
            const auditLog = new EventLog({
                ...
                pointsAwarded: action === 'add' ? amount : (action === 'remove' ? -amount : amount - oldBiWeeklyPoints),
                ...
                hrAction: {
                    ...
                    reason: reason,
                    oldBiWeeklyPoints: oldBiWeeklyPoints,
                    newBiWeeklyPoints: user.biweeklyPoints,
                    oldAllTimePoints: oldAllTimePoints,
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '📊 Biweekly Points', value: `${oldBiWeeklyPoints} → ${user.biweeklyPoints}`, inline: true },
            ...
            console.log(`   - BiWeekly: ${oldBiWeeklyPoints} → ${user.biweeklyPoints}`);
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to manage points. Please try again later.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    }

    // UPDATED: Reset biweekly statistics with enhanced completion tracking - FIXED field length limits
    static async resetCycle(interaction, confirmReset) {
        try {
            // Check HR permission
            if (!PermissionChecker.canManageSystem(interaction.member)) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('Only HR can use this command!');
            ...
            if (!confirmReset) {
                const warningEmbed = new EmbedBuilder()
                    .setColor('#ffaa00')
                    .setTitle('⚠️ Biweekly Reset Confirmation')
                    .setDescription('**WARNING: This will reset ALL biweekly statistics!**\n\nThis action will:')
                    .addFields(
                        { 
                            name: '🔄 Reset to Zero', 
                            value: '• Biweekly points\n• Biweekly events\n• Quota completion status\n• Daily points', 
                            inline: false 
                        },
                        { 
                            name: '🎯 Update Quotas', 
                        ...
                        { 
                            ...
                            value: 'Use `/reset-cycle confirm:true` to proceed', 
                            inline: false 
                        }
                    )
            ...
            const users = await MRBUser.find({}).sort({ biweeklyPoints: -1 });
            const totalUsers = users.length;
            const completedQuota = users.filter(u => u.quotaCompleted).length;
            const topPerformer = users[0];
            const totalBiweeklyPoints = users.reduce((sum, u) => sum + (u.biweeklyPoints || 0), 0);

            // Separate completed and incomplete users
            const completed = users.filter(u => u.quotaCompleted);
            const incomplete = users.filter(u => !u.quotaCompleted && u.biweeklyPoints > 0);
            ...
            // FIXED: Create biweekly champions embed with character limit handling
            const championsEmbed = new EmbedBuilder()
                .setColor('#00ff00')
                .setTitle('🏆 Biweekly Champions')
                .setDescription(`${completed.length} operator${completed.length !== 1 ? 's' : ''} completed their biweekly quota!`)
                .setTimestamp();

            if (completed.length > 0) {
                // Sort by points (highest first)
                const sortedCompleted = completed.sort((a, b) => b.biweeklyPoints - a.biweeklyPoints);
                ...
                for (let i = 0; i < sortedCompleted.length && i < 12; i++) {
                    ...
                    const bonus = user.biweeklyPoints > user.biweeklyQuota ? 
                        ` (+${user.biweeklyPoints - user.biweeklyQuota})` : '';
                    
                    const line = `${position} ${rankEmoji}**${user.username}** - ${user.biweeklyPoints} pts${bonus}`;
                ...
                if (sortedCompleted.length >= 3) {
                    championsEmbed.addFields({
                        ...
                        value: [
                            `🥇 **${sortedCompleted[0].username}** - ${sortedCompleted[0].biweeklyPoints} pts`,
                            `🥈 **${sortedCompleted[1].username}** - ${sortedCompleted[1].biweeklyPoints} pts`,
                            `🥉 **${sortedCompleted[2].username}** - ${sortedCompleted[2].biweeklyPoints} pts`
                        ].join('\n'),
                        inline: false
                    });
                }
                ...
                championsEmbed.addFields({
                    ...
                    value: 'No operators completed their quota this cycle.',
                    inline: false
                });
            }
            ...
            const incompleteEmbed = new EmbedBuilder()
                .setColor('#ff6600')
                .setTitle('⏳ Incomplete This Cycle')
                .setDescription(`${incomplete.length} operator${incomplete.length !== 1 ? 's' : ''} did not complete their biweekly quota`)
                .setTimestamp();

            if (incomplete.length > 0) {
                // Sort by percentage completion (lowest first for HR attention)
                const sortedIncomplete = incomplete.sort((a, b) => {
                    const aPercent = (a.biweeklyPoints / a.biweeklyQuota) * 100;
                    const bPercent = (b.biweeklyPoints / b.biweeklyQuota) * 100;
                ...
                for (let i = 0; i < sortedIncomplete.length && i < 12; i++) {
                    ...
                    const percentage = Math.round((user.biweeklyPoints / user.biweeklyQuota) * 100);
                    const status = percentage >= 75 ? '⚠️' : percentage >= 50 ? '⏳' : '❌';
                    const rankEmoji = RankSystem.isEliteOrHigher(user.rankLevel) ? 
                        `${RankSystem.getRankEmoji(user.rankLevel)} ` : '';
                    
                    const line = `${status} ${rankEmoji}**${user.username}** - ${user.biweeklyPoints}/${user.biweeklyQuota} pts (${percentage}%)`;
                ...
                const atRisk = sortedIncomplete.filter(u => (u.biweeklyPoints / u.biweeklyQuota) < 0.5);
                if (atRisk.length > 0) {
                    incompleteEmbed.addFields({
                ...
                incompleteEmbed.addFields({
                    name: '📊 No Activity',
                    value: 'No operators submitted events this cycle.',
                    inline: false
                });
            }

            // Apply quota reset
            console.log('🔄 Applying biweekly reset with rank-based quota system...');
            const quotaResetResult = await QuotaSystem.applyBiWeeklyQuotaReset();

            if (!quotaResetResult.success) {
                const errorEmbed = new EmbedBuilder()
                    .setColor('#ff0000')
                    .setTitle('❌ Biweekly Reset Failed')
            ...
            const resetEmbed = new EmbedBuilder()
                .setColor('#0099ff')
                .setTitle('🔄 Biweekly Reset Completed!')
                .setDescription('✅ **Successfully started new quota cycle with rank-based quotas**')
                .addFields(
                    ...
                    { name: '📊 Total Points Earned', value: totalBiweeklyPoints.toString(), inline: true },
                    { name: '🏆 Top Performer', value: topPerformer ? `${topPerformer.username} (${topPerformer.biweeklyPoints} pts)` : 'None', inline: true },
                    { name: '📈 Average Points', value: totalUsers > 0 ? (totalBiweeklyPoints / totalUsers).toFixed(1) : '0', inline: true },
                    { 
                        name: '📋 What Was Reset', 
                        value: '• Biweekly points: 0\n• Biweekly events: 0\n• Quota status: In Progress\n• Daily points: 0\n• Quotas updated by rank\n• **PRESERVED**: Rank points & progression', 
                        inline: false 
                    },
                    {
            ...
            await interaction.editReply({ 
                content: '✅ **Biweekly reset completed successfully!**',
                embeds: [championsEmbed, incompleteEmbed, resetEmbed] 
            });

            // Enhanced logging
            console.log(`🔄 BI-WEEKLY RESET: Performed by ${interaction.user.username}`);
            ...
            console.error('❌ Biweekly reset error:', error);
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to reset biweekly stats. Please try again.');
    ...
    static async viewLogs(interaction, targetUser = null, eventType = null, days = 7) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve event logs. Please try again later.');
    ...
    static async compileAuditData(targetUserId, days = 7) {
        try {
            ...
            // Get user data
            const user = await MRBUser.findOne({ discordId: targetUserId });
            if (!user) {
    ...
    static analyzeUserPerformance(events, user) {
        ...
        const quotaScore = user.quotaCompleted ? 10 : Math.min(10, (user.biweeklyPoints / user.biweeklyQuota) * 10);
        score += quotaScore;
        
        if (user.quotaCompleted) {
            strengths.push('Biweekly quota completed');
        } else {
            improvements.push('Complete biweekly quota');
        }
    ...
    static assessUserRisks(events, user, days) {
        ...
        if (!user.quotaCompleted) {
            const cycleProgress = (new Date().getDay() || 7) / 7; // Progress through current cycle
            const expectedProgress = user.biweeklyQuota * cycleProgress;
            
            if (user.biweeklyPoints < expectedProgress * 0.7) {
                warnings.push({
                    level: 'MEDIUM',
                    type: 'Quota Risk',
                    description: 'Behind expected quota progress',
                    recommendation: 'Focus on completing biweekly quota'
                });
            }
        }
