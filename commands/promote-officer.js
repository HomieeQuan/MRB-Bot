...
module.exports = {
                ...
                .addStringOption(option =>
                    option.setName('rank')
                        ...
                        .addChoices(
                            ...
                            { name: 'Gunnery Sergeant', value: 'Gunnery Sergeant' },
    ...
    async execute(interaction) {
        ...
        if (!PermissionChecker.canManageSystem(interaction.member)) {
            const errorEmbed = MRBEmbeds.createErrorEmbed('🚫 Only HR can use promotion commands!');
            ...
            if (!PermissionChecker.canForcePromotions(interaction.member)) {
                ...
                const errorEmbed = new EmbedBuilder()
                    ...
                    .addFields({
                        ...
                        value: '**HR | Commanding Officer** or higher role required for force promotions',
                        inline: false
                    })
                    .addFields({
                        name: '✅ What Changed',
    ...
    async reviewPromotion(interaction) {
        ...
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to review promotion status.');
    ...
    async approvePromotion(interaction) {
        ...
        try {
            ...
            if (!result.success) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`Failed to process promotion: ${result.error}`);
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to approve promotion.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async denyPromotion(interaction) {
        ...
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to deny promotion.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async listEligibleUsers(interaction) {
        try {
            ...
            if (report.eligibleUsers && report.eligibleUsers.length > 0) {
                ...
                try {
                    const manualCheck = await MRBUser.find({ promotionEligible: true }).limit(10);
    ...
    async forcePromote(interaction) {
        ...
        try {
            ...
            if (!PermissionChecker.canForcePromotions(interaction.member)) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('🚫 You do not have permission to use force promotions!');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            if (!targetRank) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('Invalid rank specified.');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            if (targetRank.level === user.rankLevel) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('User is already at that rank.');
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to force promote user.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async bypassRankLock(interaction) {
        ...
        try {
            ...
            if (lockStatus.expired || !user.rankLockUntil) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`${user.username} is not currently rank locked.`);
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to bypass rank lock.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async setUserRank(interaction) {
        ...
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to set user rank.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    }
};