...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (!PermissionChecker.canManageSystem(interaction.member)) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('🚫 Only HR can use the audit system!');
    ...
    async gatherAuditData(targetUser, days, interaction) {
        try {
            ...
            const weeklyRank = await MRBUser.countDocuments({ 
            ...
            .addFields(
                {
                    ...
                    value: [
                        ...
                        `**Weekly:** ${user.biweeklyPoints}/${user.biweeklyQuota} pts (${user.quotaCompleted ? '✅ Complete' : '⏳ In Progress'})`,
