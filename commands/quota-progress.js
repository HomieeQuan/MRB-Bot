...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('View biweekly quota progress for all operators')
        ...
        try {
            ...
            if (!PermissionChecker.canViewLeaderboard(interaction.member)) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .setDescription('You need the **Motorized Rifle Brigade** role to view quota progress!')
                    .setTimestamp();
            ...
            const showIncomplete = interaction.options.getBoolean('show-incomplete') ?? false;
            const showCompleted = interaction.options.getBoolean('show-completed') ?? false;

            console.log(`📊 Quota progress requested by ${interaction.user.username}`);

            // UPDATED: Get all ACTIVE users with activity this cycle
            let users = await MRBUser.find({
                active: { $ne: false }, // Only active users (filters out deleted)
                $or: [
                    { biweeklyPoints: { $gt: 0 } },
                    { quotaCompleted: true }
                ]
            }).sort({ quotaCompleted: -1, biweeklyPoints: -1 });
            ...
            if (users.length === 0) {
                const emptyEmbed = new EmbedBuilder()
                    .setColor('#ffaa00')
                    .setTitle('📊 biweekly Quota Progress')
                    .setDescription('No active operators found with activity this cycle.')
                    .addFields({
                        name: '💡 Tip',
    ...
    async createQuotaProgressEmbed(users, showIncomplete, showCompleted) {
        ...
        const totalPoints = users.reduce((sum, u) => sum + u.biweeklyPoints, 0);
    ...
    addUsersToEmbed(embed, users, titlePrefix, isCompleted) {
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            if (isCompleted) {
                ...
                const progressBar = ProgressBarGenerator.createMiniProgressBar(user.biweeklyPoints, user.biweeklyQuota, 10);
                allLines.push(`${rankEmoji}**${user.username}**\n${progressBar} ${user.biweeklyPoints}/${user.biweeklyQuota} pts${bonus}`);
            } else {
                // Incomplete users
                const percentage = Math.round((user.biweeklyPoints / user.biweeklyQuota) * 100);
                const remaining = user.biweeklyQuota - user.biweeklyPoints;
                ...
                const progressBar = ProgressBarGenerator.createMiniProgressBar(user.biweeklyPoints, user.biweeklyQuota, 10);
                allLines.push(`${status} ${rankEmoji}**${user.username}**\n${progressBar} ${user.biweeklyPoints}/${user.biweeklyQuota} pts (${percentage}%) - ${remaining} needed`);
            }
        }
