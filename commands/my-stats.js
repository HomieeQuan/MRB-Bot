...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('my-stats')
        .setDescription('View your MRB performance statistics')
    ...
    async execute(interaction) {
        ...
        if (targetUser) {
            ...
            if (!PermissionChecker.canViewOwnStats(interaction.member)) {
                const errorMessage = PermissionChecker.getPermissionErrorMessage('member');
                const errorEmbed = new EmbedBuilder()
                    .setColor('#ff0000')
                    .setTitle('🚫 Access Denied')
                    .setDescription(errorMessage)
                    .addFields({
                        name: '🎯 Required Role',
                        value: 'You need the **Motorized Rifle Brigade** role to view your MRB statistics.',
                        inline: false
                    })
                    .setFooter({ text: 'Join the MRB team to track your performance!' })
                    .setTimestamp();
