...
module.exports = {
    ...
    async execute(interaction) {
        // ===== ADMIN PERMISSION CHECK - Requires '.' role =====
        if (!PermissionChecker.canUseAdminCommands(interaction.member)) {
            const errorMessage = PermissionChecker.getPermissionErrorMessage('admin');
            const errorEmbed = new EmbedBuilder()
                ...
                .addFields({
                    ...
                    value: '**A | MRB Commander** or **Admin** role required for emergency commands',
                    inline: false
                })
    ...
    async backupUsers(interaction) {
        try {
            ...
            const users = await MRBUser.find({}).sort({ allTimePoints: -1 });
            ...
            const backup = {
                ...
                users: users.map(u => ({
                    ...
                    username: u.username,
                    biweeklyPoints: u.biweeklyPoints,
                    allTimePoints: u.allTimePoints,
                    biweeklyQuota: u.biweeklyQuota,
                    quotaCompleted: u.quotaCompleted,
                    isBooster: u.isBooster,
                    totalEvents: u.totalEvents,
                    biweeklyEvents: u.biweeklyEvents,
            ...
            const fileName = `mrb_users_backup_${new Date().toISOString().split('T')[0]}.json`;
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to create user backup');
    ...
    async backupEvents(interaction) {
        try {
            ...
            const backupText = JSON.stringify(backup, null, 2);
            const fileName = `mrb_events_backup_${days}days_${new Date().toISOString().split('T')[0]}.json`;
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to create event backup');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async emergencyReset(interaction) {
        ...
        try {
            ...
            const userCount = await MRBUser.countDocuments();
            const eventCount = await EventLog.countDocuments();
            
            // Perform the reset
            await MRBUser.deleteMany({});
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Emergency reset failed - data may be in inconsistent state!');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async fixBoosterStatus(interaction) {
        ...
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to fix booster status');
    ...
    async getDatabaseStats(interaction) {
        try {
            ...
            const userCount = await MRBUser.countDocuments();
            const eventCount = await EventLog.countDocuments();
            const boosterCount = await MRBUser.countDocuments({ isBooster: true });
            const quotaCompleted = await MRBUser.countDocuments({ quotaCompleted: true });
            ...
            const totalBiWeeklyPoints = allUsers.reduce((sum, u) => sum + u.weeklyPoints, 0);
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '📊 BiWeekly Points', value: totalBiWeeklyPoints.toString(), inline: true },
                    { name: '⭐ All-Time Points', value: totalAllTimePoints.toString(), inline: true },
                    { name: '📈 Avg Points/User', value: userCount > 0 ? Math.round(totalBiWeeklyPoints / userCount).toString() : '0', inline: true },
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve database statistics');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async fixQuotaStatus(interaction) {
        try {
            ...
            if (!result.success) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`Failed to update quotas: ${result.error}`);
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to fix quota statuses');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async fixRanks(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to fix user ranks');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async performHealthCheck(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to perform health check');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async basicHealthCheck() {
        ...
        try {
            ...
            const negativeUsers = await MRBUser.countDocuments({
                $or: [
                    { biweeklyPoints: { $lt: 0 } },
                    { allTimePoints: { $lt: 0 } }
                ]
