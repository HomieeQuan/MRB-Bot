...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('stats')
                .setDescription('View automation statistics (last 14 days)'))
        .addSubcommand(subcommand =>
            subcommand
                .setName('health')
    ...
    async notifyAdminsOfUsage(interaction, subcommand) {
        try {
            ...
            const commanderRole = guild.roles.cache.find(role => role.name === 'A | MRB Commander');
    ...
    async manualNotifyAdmins(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to send admin notifications');
    ...
    async triggerAutomation(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to trigger daily automation');
    ...
    async showSchedulerStatus(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve scheduler status');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async showAutomationStats(interaction) {
        try {
            ...
            if (!stats) {
                const errorEmbed = MRBEmbeds.createErrorEmbed('No automation statistics available. Automation may not have run yet.');
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription('Performance metrics for the last 14 days')
                .addFields(
                    {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve automation statistics');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async performHealthCheck(interaction) {
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Health check failed - system may be experiencing issues');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async testNotifications(interaction) {
        try {
            ...
            const guild = interaction.guild;
            const hrRoles = [
                guild.roles.cache.find(role => role.name === 'Commanding Officer'),
                guild.roles.cache.find(role => role.name === 'Senior Commanding Officer'),
                guild.roles.cache.find(role => role.name === 'A | Chief of Operations'),
                guild.roles.cache.find(role => role.name === 'A | Deputy Commander'),
                guild.roles.cache.find(role => role.name === 'A | MRB Commander')
            ...
            const testEmbed = new EmbedBuilder()
                .setColor('#0099ff')
                .setTitle('🧪 Notification System Test')
                .setDescription('This is a test notification from the MRB bot automation system.')
                .addFields({
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Notification test failed');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
