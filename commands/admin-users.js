...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('admin-users')
        .setDescription('Manage MRB team members (HR only)')
    ...
    async deleteUser(interaction) {
        ...
        try {
            ...
            if (!user) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`${targetUser.username} not found in database.`);
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }

            if (!user.active) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`${user.username} has already been deleted.`);
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            const userStats = {
                ...
                biweeklyPoints: user.biweeklyPoints,
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to delete user. Please try again.');
    ...
    async restoreUser(interaction) {
        ...
        try {
            ...
            if (user.active) {
                const errorEmbed = MRBEmbeds.createErrorEmbed(`${user.username} is already active.`);
                return await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
            }
            ...
            // Keep all-time points, they start fresh with biweekly/rank
            await user.save();
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to restore user.');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    async cleanupInactiveUsers(interaction) {
        ...
        try {
            ...
            const inactiveUsers = await MRBUser.find({
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed(`Failed: ${error.message}`);
    ...
    async bulkDeleteLeftServer(interaction) {
        ...
        try {
            ...
            const allUsers = await MRBUser.find({ active: { $ne: false } }); // Only check active users
    ...
    async listInactiveUsers(interaction) {
        ...
        try {
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve inactive users');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async listDeletedUsers(interaction) {
        try {
            ...
            const deletedUsers = await MRBUser.find({ active: false }).sort({ deletedAt: -1 });
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve deleted users');
            await interaction.editReply({ embeds: [errorEmbed] });
        }
    },
    ...
    async getUserStats(interaction) {
        try {
            const totalUsers = await MRBUser.countDocuments({ active: { $ne: false } });
            const deletedUsers = await MRBUser.countDocuments({ active: false });
            const totalEvents = await EventLog.countDocuments();
            
            const cycleAgo = new Date();
            cycleAgo.setDate(cycleAgo.getDate() - 7);
            const activeLastCycle = await EventLog.distinct('userId', {
                submittedAt: { $gte: cycleAgo },
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '🔥 Active (14 days)', value: activeLastCycle.length.toString(), inline: true },
                    { name: '📈 Active (30 days)', value: activeLastMonth.length.toString(), inline: true },
                    { name: '📊 Activity Rate', value: `${Math.round((activeLastCycle.length / totalUsers) * 100)}%`, inline: true }
                )
            ...
            const errorEmbed = MRBEmbeds.createErrorEmbed('Failed to retrieve user statistics');
            await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
        }
    },
    ...
    getTimeSince(date) {
        ...
        if (diffDays < 30) return `${Math.floor(diffDays / 7)} cycles`;
