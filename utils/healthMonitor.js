...
class HealthMonitor {
    ...
    static async checkDataIntegrity() {
        try {
            ...
            const quotaInconsistent = await MRBUser.countDocuments({
                $expr: {
                    $ne: [
                        '$quotaCompleted',
                        { $gte: ['$biweeklyPoints', '$biweeklyQuota'] }
                    ]
                }
            });
            ...
            const userIds = await MRBUser.distinct('discordId');
    ...
    static async checkPerformance() {
        try {
            ...
            await MRBUser.find({}).sort({ biweeklyPoints: -1 }).limit(10);
    ...
    static async checkRecentActivity() {
        try {
            ...
            const oneCycleAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
            
            const recentEvents = await EventLog.countDocuments({
                submittedAt: { $gte: oneDayAgo }
            });
            
            const biweeklyEvents = await EventLog.countDocuments({
                submittedAt: { $gte: oneCycleAgo }
            });
            ...
            if (biweeklyEvents < 10) warnings.push('Low activity this cycle');
            
            return {
                status: warnings.length === 0 ? 'healthy' : 'warning',
                details: {
                    eventsToday: recentEvents,
                    eventsThisCycle: biweeklyEvents,
                    activeUsersThisCycle: activeUsers.length,
                    warnings
                }
            };
    ...
    static async generateHealthReport() {
        ...
        let report = `🏥 MRB Bot Health Report - ${health.timestamp}\n`;
