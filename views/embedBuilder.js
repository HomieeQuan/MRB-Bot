...
class MRBEmbeds {
    ...
    static createPersonalStatsEmbed(user, discordUser, weeklyRank, allTimeRank, recentEvents = [], isPersonalStats = true) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle(`📊 ${user.username}'s MRB Statistics`)
            ...
            if (user.rankLevel >= 9) {
                ...
                embed.addFields({
                    ...
                    value: ProgressBarGenerator.createQuotaProgressBar(user.biweeklyPoints, user.biweeklyQuota),
                    inline: false
                });
            }
            ...
            embed.addFields(
                ...
                { 
                    ...
                    value: `${user.biweeklyEvents} events`, 
                    inline: true 
                },
                { 
                    name: '🔥 Points Today', 
    ...
    static async createEnhancedLeaderboardEmbed(users, type = 'weekly', enhancedStats = null) {
        try {
            ...
            const embed = new EmbedBuilder()
                .setColor('#00ff00')
                .setTitle(`MRB ${type === 'biweekly' ? 'BiWeekly' : 'All-Time'} Leaderboard`)
                .setTimestamp();

            // Filter active users (with points > 0) and limit to 25
            const activeUsers = users.filter(user => {
                const points = type === 'biweekly' ? user.biweeklyPoints : user.allTimePoints;
                return points > 0;
            }).slice(0, 25);
    ...
    static createEventSubmissionEmbed(user, eventType, description, actualPoints, basePoints, screenshot) {
        const embed = new EmbedBuilder()
            .setColor(user.quotaCompleted ? '#00ff00' : '#ffa500')
        ...
        if (user.quotaCompleted && user.biweeklyPoints - actualPoints < user.biweeklyQuota) {
    ...
    static createNoStatsEmbed() {
        return new EmbedBuilder()
            ...
            .setDescription('You haven\'t submitted any events yet! Use `/submit-event` to get started and begin tracking your MRB performance.')
            .addFields(
                { name: '🎯 BiWeekly Quota', value: '10 points', inline: true },
